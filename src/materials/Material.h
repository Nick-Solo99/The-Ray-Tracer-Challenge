...
namespace rtc::materials {
    ...
    struct Material {
        ...
        Material() = default;
        Material(const Color& color, const float ambient, const float diffuse, const float specular, const float shininess, const float reflective, std::unique_ptr<Pattern> pattern = nullptr)
        : color(color), ambient(ambient), diffuse(diffuse), specular(specular), shininess(shininess), reflective(reflective), pattern(std::move(pattern)) {}
        Material(const Material& other)
        : color(other.color), ambient(other.ambient), diffuse(other.diffuse), specular(other.specular), shininess(other.shininess),
        reflective(other.reflective), pattern(other.pattern ? other.pattern->clone() : nullptr){}
        ...
        bool operator==(const Material& other) const { return color == other.color && ambient == other.ambient && diffuse == other.diffuse && specular == other.specular && shininess == other.shininess && pattern == other.pattern && reflective == other.reflective; }
